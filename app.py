...
def round_up_time(dt: datetime, minute_step: int = 10) -> datetime:
    """
    Retourne un datetime arrondi au prochain multiple de `minute_step` minutes.
    """
...
# --- GESTION DE LA LANGUE (sidebar, affichée avant la demande de géoloc) ---
...
def init_session_state(lat=None, lon=None):
    if 'initialized' not in st.session_state:
        ...
        st.session_state.show_vl_menu = False
...
# --- DEMANDE / RÉCUPÉRATION DE LA GÉOLOCALISATION ---
location_data = streamlit_geolocation()

# Initialize session after we have (or not) location
if location_data and location_data.get('latitude') is not None:
    init_session_state(location_data['latitude'], location_data['longitude'])

# --- SI ON A LA GÉOLOCALISATION / AFFICHAGE PRINCIPAL ---
if location_data and location_data.get('latitude') is not None:
    # --- BARRE LATÉRALE (SIDEBAR) : contenu additionnel (langue déjà placé plus haut) ---
    with st.sidebar:
        st.markdown("---")
    ...
    # --- SECTION RECOMMANDATION ET VIRTUAL LINE ---
    ...
    if st.button(t("find_best_button"), type="primary", use_container_width=True):
        if not st.session_state.attractions_to_visit:
            ...
            with st.spinner(t("analysis_in_progress")):
                ...
                # Efface la recommandation si aucune n'est possible
                if recommendation['cost'] == float('inf'):
                    st.session_state.last_recommendation = None

    st.markdown("---")
    if not st.session_state.show_vl_menu:
        if st.button(t("manage_vl_button"), use_container_width=True):
            st.session_state.show_vl_menu = True
            st.rerun()
    else:
        # --- FORMULAIRE VL : robustification + default futur arrondi ---
        with st.form("virtual_line_form"):
            st.subheader(t("vl_form_header"))
            ...
            # Build a robust default_time that is in the near future (rounded up)
            fuseau_horaire_parc = pytz.timezone('Europe/Berlin')
            now = datetime.now(fuseau_horaire_parc)
            rounded_dt = round_up_time(now, minute_step=10)
            default_time = rounded_dt.time()

            # If there's already stored VL info, prefer that
            if "time" in st.session_state.virtual_line_details:
                stored_time = st.session_state.virtual_line_details["time"]
                if isinstance(stored_time, datetime):
                    try:
                        stored_local = stored_time.astimezone(fuseau_horaire_parc)
                        default_time = stored_local.time()
                    except Exception:
                        default_time = stored_time.time()
                elif isinstance(stored_time, time):
                    default_time = stored_time
            elif "time_only" in st.session_state.virtual_line_details:
                default_time = st.session_state.virtual_line_details["time_only"]
            elif "time_str" in st.session_state.virtual_line_details:
                try:
                    default_time = datetime.strptime(st.session_state.virtual_line_details["time_str"], "%H:%M").time()
                except Exception:
                    default_time = rounded_dt.time()
            ...
            with confirm_col:
                if st.form_submit_button(t("vl_confirm_button"), use_container_width=True, type="primary"):
                    ...
                    try:
                        ...
                        appointment_tz = appointment_naive.replace(tzinfo=fuseau_horaire_parc)

                    if appointment_tz <= now:
                        appointment_tz = appointment_tz + timedelta(days=1)

                    st.session_state.virtual_line_active = True
                    st.session_state.virtual_line_details = {
                        "attraction": vl_attraction,
                        "time": appointment_tz,                        # datetime tz-aware
                        "time_only": appointment_tz.time(),            # datetime.time
                        "time_str": appointment_tz.strftime('%H:%M'), # stable display string
    ...
    if st.session_state.virtual_line_active and not st.session_state.show_vl_menu:
        vl = st.session_state.virtual_line_details
        vl_time_str = vl.get('time_str')
        if not vl_time_str and isinstance(vl.get('time'), datetime):
            vl_time_str = vl['time'].strftime('%H:%M')
        st.info(t("vl_active_info", attraction=vl['attraction'], time=vl_time_str))
    ...
    if st.session_state.all_candidates:
        with st.expander(t("show_details_expander"), expanded=False):
            for candidate in st.session_state.all_candidates:
                ...
                if is_late:
                    st.markdown("<div style='color: lightgrey;'>", unsafe_allow_html=True)
                ...
                delta_value, delta_color = None, "off"
                if real_wait == "CLOSED":
                    ...
                    if isinstance(predicted_wait, (int, float)) and real_wait != "CLOSED":
                        ...
                        delta_value = f"{diff:.0f} {t('minutes_abbr')}"
                        delta_color = "inverse"
                cols[1].metric(t("wait_details"), metric_value, delta=delta_value, delta_color=delta_color)
                
                if st.session_state.virtual_line_active:
                    arrival_time_vl = candidate.get('arrival_at_vl')
                    arrival_display = None
                    if isinstance(arrival_time_vl, datetime):
                        arrival_display = arrival_time_vl.strftime('%H:%M')
                    elif isinstance(arrival_time_vl, time):
                        arrival_display = arrival_time_vl.strftime('%H:%M')
                    elif isinstance(arrival_time_vl, str):
                        arrival_display = arrival_time_vl
                    else:
                        arrival_display = t("not_applicable_abbr")
                    cols[2].metric(t("end_time_vl_details"), arrival_display)
                else:
...
# --- CAS OÙ L'UTILISATEUR N'A PAS ENCORE DONNÉ LA GÉOLOCALISATION ---
else:
